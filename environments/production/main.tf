...
module "cluster" {
  ...
  host_port    = 8081   # different port so both can run locally
}
...
output "argocd_port_forward_command" {
  value = "kubectl --context ${module.cluster.kubeconfig_context} -n ${module.argocd.namespace} port-forward svc/argocd-server 8444:443"
}
